...
function 下载bat(域名,token) {
	return [
	  ...
	  `set "FILENAME=%~nx1"`,
	  ``,
	  `rem PowerShell命令读取文件的前99999行内容，将内容转换为UTF8并进行base64编码`,
	  `for /f "delims=" %%i in ('powershell -command "$content = ((Get-Content -Path '%cd%/%FILENAME%' -Encoding UTF8) | Select-Object -First 99999) -join [Environment]::NewLine; [convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($content))"') do set "BASE64_TEXT=%%i"`,
	  ``,
...
BASE64_TEXT=$(head -n 99999 $FILENAME | base64)
	...
	return `
	  <html>
		...
		<body>
		  ...
		  token: ${token} <br>
		  <br>
		  <pre>注意! 因URL长度内容所限，脚本更新方式一次最多更新99999行内容</pre><br>
